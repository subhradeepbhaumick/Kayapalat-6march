...
const BusinessBrandAddProduct = ({
  ...
  useEffect(() => {
    ...
    if (finalProductCost >= sellingMrp && sellingMrp > 0) {
      setCostError("Your Product's Selling Cost should not exceed the MRP. Please reduce the Selling Cost.");
    } else {
      setCostError(null);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formDataToSend.append("commissionPercentage", commissionPercentage.toString());
  ...
  return (
    <div className="max-w-4xl mx-auto p-3 sm:p-4 lg:p-6">
      <div className="bg-white rounded-lg sm:rounded-xl shadow-lg overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-3 sm:p-4 lg:p-8 space-y-4 sm:space-y-6 lg:space-y-8">
                ...
                <div className="space-y-2">
                  <div className="relative">
                    ...
                    <select
                      ...
                      disabled={mode === "view"}
                      className={`w-full pl-10 pr-3 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#295A47] focus:border-transparent appearance-none bg-white ${
            ...
            <div className="space-y-4 sm:space-y-6">
              ...
              <div className="p-3 sm:p-4 bg-white rounded-lg border">
                ...
                <div className="mt-2 p-2 bg-blue-50 rounded-lg border border-blue-200">
                  <p className="text-xs text-blue-700">
                    For discussion about commission(%), contact MR. John Bor - 7044400100
                  </p>
                </div>
                {commissionPercentage > 0 && (
                  <div className="mt-3 p-3 bg-orange-50 rounded-lg border border-orange-200">
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      Commission Charge ({commissionPercentage}%) + 18% GST
                    </label>
                    <div className="relative">
                      <span className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 text-sm">
                        ₹
                      ...
                      <input
                        type="text"
                        value={(() => {
                          const commission = (parseFloat(formData.mrp) || 0) * (commissionPercentage / 100);
                          const gstOnCommission = commission * 0.18;
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between p-3 sm:p-4 bg-white rounded-lg border gap-3 sm:gap-0">
                ...
                <button
                  ...
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                    gstIncluded ? "bg-green-600" : "bg-orange-600"
                  } ${mode === "view" ? "opacity-50 cursor-not-allowed" : ""}`}
                >
                  <span
                    className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                      gstIncluded ? "translate-x-6" : "translate-x-1"
                    }`}
                  />
                </button>
              </div>

              <div className={`ml-0 sm:ml-4 p-3 rounded-lg border mt-3 sm:mt-0 ${gstIncluded ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  GST Percentage (%)
                </label>
                <input
                  ...
                  placeholder="18"
                  disabled={gstIncluded || mode === "view"}
                  className={`w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#295A47] focus:border-transparent [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:m-0 [&::-webkit-inner-spin-button]:m-0 [&::-webkit-outer-spin-button]:-webkit-appearance-none [&::-webkit-inner-spin-button]:-webkit-appearance-none [&::-moz-appearance]:textfield [&::-webkit-outer-spin-button]:hidden [&::-webkit-inner-spin-button]:hidden [&::-webkit-outer-spin-button]:invisible [&::-webkit-inner-spin-button]:invisible ${gstIncluded || mode === "view" ? "bg-gray-100 cursor-not-allowed" : ""}`}
                />
                <div className="mt-3">
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    GST Amount
                  ...
                  <div className="relative">
                    ...
                    <input
                      type="text"
                      value={(gstIncluded ? (((parseFloat(formData.mrp) || 0) * 100) / (100 + (parseFloat(gstPercentage) || 0))) * ((parseFloat(gstPercentage) || 0) / 100) : ((parseFloat(formData.mrp) || 0) * (parseFloat(gstPercentage) || 0) / 100)).toFixed(2)}
                      disabled
                      className="w-full pl-10 pr-3 py-2 border border-gray-300 rounded-lg bg-gray-100 text-gray-600"
                    />
                  </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between p-3 sm:p-4 bg-white rounded-lg border gap-3 sm:gap-0">
                ...
                <button
                  ...
                  disabled={mode === "view"}
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                    transportationIncluded ? "bg-green-600" : "bg-orange-600"
                  } ${mode === "view" ? "opacity-50 cursor-not-allowed" : ""}`}
                >
                  <span
                    className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                      transportationIncluded ? "translate-x-6" : "translate-x-1"
                    }`}
                  />
                </button>
              </div>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center gap-2 sm:gap-0">
                  <div className="w-full sm:w-auto flex justify-between sm:block items-center">
                    ...
                    <div className="text-lg font-bold text-[#295A47]">
                      ₹{gstIncluded ? parseFloat(formData.mrp || "0").toFixed(2) : (parseFloat(formData.mrp || "0") + ((parseFloat(formData.mrp) || 0) * (parseFloat(gstPercentage) || 0) / 100) + (!transportationIncluded && !discussTransportationLater ? parseFloat(transportationCost) || 0 : 0)).toFixed(2)}
                    </div>
                  </div>
