...
const BusinessBrandMyListing = ({
  ...
  const filteredProducts = products.filter((product) => {
    ...
    if (filter === "Showroom Stock") return product.showroom_stock === 1 || product.showroom_stock === 2;
    return true;
  });
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <div className="flex flex-wrap gap-2 sm:gap-4">
            ...
            <button
              onClick={() => setFilter("Showroom Stock")}
              className={`px-3 sm:px-4 py-2 rounded-lg font-medium transition-colors text-sm sm:text-base ${
                filter === "Showroom Stock"
                  ? "bg-[#295A47] text-white"
                  : "bg-white text-gray-700 border border-gray-300 hover:bg-gray-50"
              }`}
            >
              Showroom Stock ({products.filter((p) => p.showroom_stock === 1 || p.showroom_stock === 2).length})
            </button>
          </div>
        </div>
        {/* Products Grid */}
                ...
                <div className="relative h-48 bg-gray-100 overflow-hidden">
                  {product.images && product.images.length > 0 ? (
                    <Image
                      ...
                      alt={
                        product.images[0].image_alt_text || product.product_name
                      }
                      fill
                  ...
                  <div className="absolute top-3 left-3 bg-white/90 backdrop-blur-sm rounded-lg p-2">
                    <label className="flex items-center gap-2 cursor-pointer">
                      ...
                      <span className="text-xs font-semibold text-gray-700">
                        {stockStatus[product.product_id] ?? true
                          ? "In Stock"
                          : "Out of Stock"}
                      </span>
                    </label>
                ...
                <div className="p-4">
                  ...
                  <div className="flex flex-col gap-1 mb-3">
                    ...
                    <span className="text-sm text-red-600">
                      Including Commission, GST
                      {product.transport_exclude === 1
                        ? ". Transportation cost to be discussed."
                        : ", Transportation cost."}
                    </span>
                    {/* Showroom Stock Section */}
                    <div className="mt-3 border-t pt-3 space-y-2">
                      {/* Hide checkbox if already confirmed */}
                      {product.showroom_stock !== 2 && (
                        <label className="flex items-center gap-2 cursor-pointer">
                          <input
                            type="checkbox"
                            checked={product.showroom_stock === 1}
                            onChange={async (e) => {
                              const newValue = e.target.checked ? 1 : 0;
                              try {
                                const res = await fetch(
                                  "/api/businessBrand/products/showroom_stock",
                                  {
                                    method: "PUT",
                                    headers: {
                                      "Content-Type": "application/json",
                                ...
                                if (res.ok) {
                                  ...
                                  toast.error(data.error || "Failed to update showroom stock");
                                }
                              } catch (error) {
                                toast.error("Error updating showroom stock");
                              }
                            }}
                            className="w-4 h-4 text-[#295A47] border-gray-300 rounded focus:ring-[#295A47]"
                          />
                          <span className="text-sm font-medium text-gray-700">
                            Request Showroom Stock
                          </span>
                        </label>
                      )}
                      {/* Status Badge */}
                  ...
                  <p className="text-sm text-gray-600 line-clamp-2 mb-4">
                    {truncateText(
                      product.short_description || product.about_product || "",
                      80
                    )}
                  </p>
                  {/* Additional Info */}
