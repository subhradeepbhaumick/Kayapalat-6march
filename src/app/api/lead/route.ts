...
export async function POST(req: NextRequest) {
  try {
    ...
    const leadType = data.pickup_type === "For Sale" ? 1 : 0;
    ...
    try {
      ...
      if (data.lead_id) {
        ...
        const insertLeadsQuery = `
          INSERT INTO leads
          (lead_id, admin_id, client_name, client_phone, email, whatsapp, address, city, state, pincode, lead_date,lead_type)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `;
      ...
      const insertProjectsQuery = `
        ...
        (appointment_id, lead_id, agent_id, admin_id, client_name, client_phone, location, project_value, commission, agent_share, details, property_type, created_at,lead_type)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `;
...
export async function GET(req: NextRequest) {
  try {
    ...
    const userRows = await pool.query('SELECT user_id FROM users_kp_db WHERE user_id = ? AND role IN (?, ?)', [userId, 'referuser', 'client']);
