...
export async function GET(request: NextRequest) {
  try {
    ...
    SELECT composite_gst_scheme FROM manufacturer WHERE dealer_id = ?`, [businessBrandId]);
    const compositeGstScheme = brandResult?.length > 0 && brandResult[0].composite_gst_scheme === 1;
...
const showroomStockNumber = formData.get('showroomStockNumber') as string || '';
    ...
    if (compositeGstScheme) {
      // 🚫 Composite scheme → NO GST
      gst_percentage = 0;
      gst_amount = 0;
      gst_exclude = 0;
    } else {
      gst_percentage = gstPercentage;
      gst_exclude = gstIncluded ? 0 : 1;
      gst_amount = gstIncluded
        ? ((mrp * 100) / (100 + gstPercentage)) * (gstPercentage / 100)
        : mrp * (gstPercentage / 100);
    }
    const transportation_cost = transportExclude ? 0 : (transportationIncluded ? 0 : transportationCost);
    const base_mrp = mrp;
    const final_product_cost = compositeGstScheme
      ? (mrp + commission_amount + transportation_cost)
      : gstIncluded
        ? (mrp + commission_amount + transportation_cost)
        : (mrp + commission_amount + gst_amount + transportation_cost);
    ...
    const [insertResult] = await executeQuery(`
      INSERT INTO product_details (
        ...
        transportation_cost, transport_exclude, base_mrp, final_product_cost,showroom_stock_number, is_active, created_at, updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?, 1, NOW(), NOW())
    `, [
      businessBrandId, category, productName, description, aboutProduct, productType, sellingMrp, mrp,
      commission_percentage, commission_amount, gst_percentage, gst_exclude, gst_amount,
      transportation_cost, transportExclude, base_mrp, final_product_cost,showroomStockNumber
    ]);
...
const showroomStockNumber = formData.get('showroomStockNumber') as string || '';
    ...
    await executeQuery(`
      UPDATE product_details SET
        ...
        commission_percentage = ?, commission_amount = ?, gst_percentage = ?, gst_exclude = ?,gst_amount = ?,
        transportation_cost = ?, transport_exclude = ?, base_mrp = ?, final_product_cost = ?, showroom_stock_number = ?,updated_at = NOW()
      WHERE product_id = ? AND dealer_id = ?
    `, [
      category, productName, description, aboutProduct, productType, sellingMrp, mrp,
      commission_percentage, commission_amount, gst_percentage,gst_exclude, gst_amount,
      transportation_cost, transportExclude, base_mrp, final_product_cost,showroomStockNumber, productId, businessBrandId
    ]);
