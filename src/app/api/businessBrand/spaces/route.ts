  ...
  try {
    ...
    if (spaceType) {
      ...
      const [rows] = await connection.execute(`
        SELECT space_type, size, price, dealer_id, client_name,due
        FROM \`showroom a\`
        WHERE space_type = ?
      `, [spaceType]);
      ...
      const [rows] = await connection.execute(`
        SELECT space_type, size, price, booking_status
        FROM \`showroom a\`
      `);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const [updateResult]: any = await connection.execute(`
      UPDATE \`showroom a\`
      SET dealer_id = ?, client_name = ?, advance = ?, booking_cost = ?, due=?, time_period = ?, transaction_proof = ?, booking_status = 'pending', booking_date = CURDATE(), expire_date = DATE_ADD(CURDATE(), INTERVAL ? MONTH)
      WHERE space_type = ? AND dealer_id IS NULL
    `, [dealer_id, company_name, advance, booking_cost, due, time_period, transactionProofPath, timePeriodInt, space_type]);
