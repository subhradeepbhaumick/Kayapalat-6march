...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const orderId of order_ids) {
      ...
      // ==============================
      const [buyRows] = await connection.execute(
        `
    ...
    `,
      ...
      const buyProducts = buyRows as any[];
      if (buyProducts.length > 0) {
        const product_id = buyProducts[0].product_id;
        const quantity = Number(buyProducts[0].quantity || 0);
        // ==============================
        // UPDATE BUY PRODUCT TABLE
        // ==============================
      ...
      SET
        showroom_stock_number =
          GREATEST(showroom_stock_number - ?, 0),
        updated_at = NOW()
      WHERE product_id = ?
      `,
          [quantity, product_id]
