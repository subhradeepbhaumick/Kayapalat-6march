  ...
  try {
    ...
    try {
      ...
      if (data.agent_id) {
        ...
        // ❌ If agent NOT exists → insert from users_kp_db
        if ((agentCheck as any).length === 0) {
          ...
          if ((userData as any).length > 0) {
            ...
            await connection.query(
              `INSERT INTO agents 
        (agent_id, admin_id, agent_name, phone, whatsapp, address, email, occupation, profile_pic, password_hash, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())`,
              [
                user.user_id,
                "S1", // or assign default admin if needed
                user.name,
                user.phone,
                user.whatsapp || "",
                user.address || "",
      ...
      let admin_id = "S1"; // default fallback

      if (data.agent_id) {
        const [agentRow] = await connection.query(
          `SELECT admin_id FROM agents WHERE agent_id = ?`,
          [data.agent_id]
        );
      ...
      if (data.lead_id) {
        ...
        const insertLeadsQuery = `
          INSERT INTO leads
          (lead_id, agent_id, admin_id, client_name, client_phone, email, whatsapp, address, city, state, pincode, lead_date)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `;
        ...
        await connection.query(insertLeadsQuery, [
          lead_id,
          loggedInUserId, // only set agent_id if the logged in user is the same as the agent_id provided
          admin_id,
          data.client_name,
          data.client_phone,
          data.email || null,
...
export async function GET(req: NextRequest) {
  try {
    ...
    const userRows = await pool.query('SELECT user_id FROM users_kp_db WHERE user_id = ? AND role IN (?, ? , ?)', [userId, 'referuser', 'client', 'metro_client']);
    ...
    // ✅ Sort by latest appointment (BEST)
    query += ` ORDER BY projects.appointment_id DESC`;
