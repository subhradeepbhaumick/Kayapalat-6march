...
export async function POST(request: NextRequest) {
  try {
    ...
    const gst_amount = formData.get('gst_amount') as string;
    ...
    // Insert new payment
    await executeQuery(`
      INSERT INTO design_fees_payments (client_id, plan_type, amount, gst_amount, total_amount, payment_method, transaction_proof_path, status)
      VALUES (?, ?, ?, ?, ?, ?, ?, 'pending')
    `, [clientId, plan_type, amount, gst_amount, total_amount, payment_method, transactionProofPath]);
