...
const ClientDesignTransactions = () => {
  ...
  const handleToggle = async (transactionId: number, currentStatus: string) => {
    ...
    try {
      ...
      const response = await fetch(`/api/superadmin/design-fees-payments/${transactionId}/${endpoint}`, {
        method: 'POST',
      });

      if (response.ok) {
        const newStatus = isApprove ? 'approved' : 'rejected';
        toast.success(`Transaction ${isApprove ? 'approved' : 'rejected'} successfully`);
        // Update the local state
        setTransactions(prev =>
          prev.map(t =>
            t.id === transactionId ? { ...t, status: newStatus as 'pending' | 'approved' | 'rejected' } : t
          )
        );
      } else {
        const error = await response.json();
        toast.error(error.error || `Failed to ${isApprove ? 'approve' : 'reject'} transaction`);
      }
    } catch (error) {
      console.error(`Error ${isApprove ? 'approving' : 'rejecting'} transaction:`, error);
            ...
            <tbody>
              {transactions.map((transaction) => (
                <tr key={transaction.id} className="hover:bg-gray-50">
                  <td className="border border-gray-300 px-4 py-2">{transaction.client_id}</td>
                  <td className="border border-gray-300 px-4 py-2">{transaction.client_name || 'N/A'}</td>
                  <td className="border border-gray-300 px-4 py-2 capitalize">{transaction.plan_type}</td>
                  <td className="border border-gray-300 px-4 py-2">₹{transaction.amount.toLocaleString('en-IN')}</td>
                  <td className="border border-gray-300 px-4 py-2">₹{transaction.gst_amount.toLocaleString('en-IN')}</td>
                  <td className="border border-gray-300 px-4 py-2">₹{transaction.total_amount.toLocaleString('en-IN')}</td>
                  <td className="border border-gray-300 px-4 py-2 capitalize">{transaction.payment_method}</td>
                  <td className="border border-gray-300 px-4 py-2">
                    {transaction.transaction_proof_path ? (
                      <img
                        ...
                        onClick={() => setModalImage(transaction.transaction_proof_path)}
                      />
                    ) : (
                      'N/A'
                    )}
                  ...
                  <td className="border border-gray-300 px-4 py-2">
                    <button
                      onClick={() => handleToggle(transaction.id, transaction.status)}
                      disabled={approving === transaction.id}
                      className={`px-3 py-1 text-white rounded hover:opacity-80 disabled:opacity-50 text-sm ${
                        transaction.status === 'approved'
                    ...
                    >
                      {approving === transaction.id
                        ? (transaction.status === 'approved' ? 'Rejecting...' : 'Approving...')
                        : transaction.status === 'approved'
                        ? 'Reject'
                        : 'Approve'
                      }
