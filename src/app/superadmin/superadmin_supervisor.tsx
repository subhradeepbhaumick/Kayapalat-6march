...
const SuperAdmin_Supervisor = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (projRes.ok) {
          ...
          setProjectAllotments(projData.projects.map((p: any) => ({
            ...p,
            supervisors: []
          })));
        }
        ...
        if (assignRes.ok && assignData.assignments) {
          setProjectAllotments(prev => prev.map(p => ({
            ...p,
  ...
  const filteredSupervisors = supervisors.filter(
    (supervisor) => selectedSupervisor === 'All' || supervisor.name === selectedSupervisor
  );

  // Add supervisor to project
  const handleAddSupervisor = async (projectId: string, supervisorId: string) => {
        ...
        setProjectAllotments(prev => prev.map(p => 
          ...
          ? { 
              ...
              supervisors: [...p.supervisors, {
                ...
                profile_pic: supervisor?.profilePic || '/placeholder_person.jpg'
              }]
            } 
          : p
        ));
  ...
  const handleRemoveSupervisor = async (projectId: string, assignmentId: number) => {
    try {
      const res = await fetch(`/api/superadmin/project-supervisor?id=${assignmentId}`, {
        method: 'DELETE'
      });
      if (res.ok) {
        // Update local state
        setProjectAllotments(prev => prev.map(p => 
          p.id === projectId 
          ? { ...p, supervisors: p.supervisors.filter(s => s.id !== assignmentId) }
          : p
        ));
      } else {
        const data = await res.json();
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (isEditing) {
          ...
          setSupervisors(prev => prev.map(s => s.user_id === editingSupervisor?.user_id ? {
            ...s,
            name: formData.name,
  ...
  const handleSupervisorSelect = (projectId: string, supervisorId: string) => {
    ...
    if (supervisor) {
      setProjectAllotments(prev => prev.map(p => 
        p.id === projectId 
        ? { ...p, supervisor_id: supervisor.user_id, supervisor_name: supervisor.name } 
        : p
      ));
      setActiveAllotmentId(null);
    }
  ...
  return (
    <div className="p-6">
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold text-[#295A47] mb-4">Supervisor Management</h1>
        <p className="text-gray-600 text-lg">Manage your supervisors and their details.</p>
      </div>

      <div className="flex items-end justify-between mb-6">
        <div>
          <label className="text-sm text-gray-600 block mb-1">Select Supervisor</label>
          <select
        ...
        <table className="min-w-full border border-gray-200 bg-white">
          ...
          <tbody>
            {filteredSupervisors.length > 0 ? filteredSupervisors.map((supervisor, index) => (
              <tr key={supervisor.user_id} className="hover:bg-green-50 transition">
                <td className="px-4 py-2 border text-center">{supervisor.user_id}</td>
                <td className="px-4 py-2 border text-center">
                  <img src={supervisor.profilePic} className="w-10 h-10 rounded-full mx-auto" />
                </td>
      ...
      <div className="mt-10">
        <h2 className="text-2xl font-bold text-[#295A47] mb-4">Project Allotment</h2>
        <div className="overflow-x-auto rounded-lg shadow">
          <table className="min-w-full border border-gray-200 bg-white">
            <thead className="bg-[#D7E7D0] text-gray-700">
              <tr>
                ...
                <tr key={project.id} className="hover:bg-green-50 transition">
                    ...
                    {activeAllotmentId === project.id ? (
                      <div className="flex flex-col gap-2">
                        ...
                        >
                          ...
                          {supervisors
                            .filter(s => !project.supervisors.some(ps => ps.supervisor_id === s.user_id))
                            .map((s) => (
                        ...
                        <button
                          ...
                          onClick={() => setActiveAllotmentId(null)}
                        >
                          Cancel
                        </button>
                      </div>
                  ...
                  <td className="px-4 py-2 border text-center">
                    <button
                      ...
                      onClick={() => setActiveAllotmentId(activeAllotmentId === project.id ? null : project.id)}
                    >
      ...
      {showAddPopup && (
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-96 relative">
            ...
            <h2 className="text-xl font-semibold mb-4 text-center">{isEditing ? 'Edit Supervisor' : 'Add New Supervisor'}</h2>

            <form onSubmit={submitSupervisor} encType="multipart/form-data" className="space-y-3">
