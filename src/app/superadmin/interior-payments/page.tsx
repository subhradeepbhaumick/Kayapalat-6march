...
const InteriorPaymentsAdmin = () => {
  ...
  const handleDownloadPDF = async () => {
    ...
    pdf.save(`Ledger-${selectedProject?.project_name}.pdf`);
  };
  ...
  return (
    <div className="min-h-screen bg-[#D2EBD0] flex">
      {/* Sidebar */}
      <div
        className={`fixed inset-y-0 left-0 z-50 bg-white shadow-lg transition-all duration-300 ${sidebarCollapsed ? "w-0 lg:w-16" : "w-64"
          }`}
      >
        <div className="p-4 border-b bg-[#D7E7D0]">
          <div className="flex items-center justify-between">
            {!sidebarCollapsed && (
              <h2 className="text-lg font-semibold text-[#295A47]">
                Interior Payments
              </h2>
            )}
        ...
        <nav className="mt-8">
          {sidebarItems.map((item) => (
            <div
              ...
              className={`flex items-center py-3 cursor-pointer ${sidebarCollapsed ? "justify-center px-3" : "px-6"
                } ${activeTab === item.key
      ...
      {/* Main Content */}
      <div
        className={`flex-1 overflow-y-auto overflow-x-hidden pt-20 ${sidebarCollapsed ? "lg:ml-16" : "lg:ml-64"
          }`}
      >
                  ...
                  <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start gap-4 mb-8">
                    <div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-2 sm:gap-4 text-sm">
                        ...
                        <div>
                          GST: ₹
                          {(selectedProject.gst_total ?? 0).toLocaleString(
                            "en-IN"
                          )}
                        </div>
                        <div>
                      ...
                      <tbody>
                        {ledger.map((entry) => (
                          ...
                          >
                            ...
                            <td className="border border-gray-300 px-4 py-2">
                              <span
                                className={`px-2 py-1 rounded-full text-xs font-medium capitalize ${entry.status === "confirmed"
                      ...
                      {(addForm.entry_type === "charge" ||
                        addForm.entry_type === "extra_work") && (
                          <div>
                            ...
                            <select
                              value={
                                addForm.gst_included ? "including" : "excluding"
                              }
                              onChange={(e) =>
                                setAddForm((prev) => ({
                                  ...prev,
                                  gst_included: e.target.value === "including",
                                }))
                              }
                              className="w-full px-3 py-2 border border-gray-300 rounded-lg"
                            >
                        ...
                        <div className="text-sm text-gray-700 bg-gray-50 border border-gray-200 rounded-lg p-3">
                          {(addForm.entry_type === "charge" ||
                            ...
                            <>
                              <div>
                                Total (including GST): ₹
                                {getGstPreview()!.totalAmount.toLocaleString(
                                  "en-IN"
                                )}
                              </div>
                              <div>
                                GST: ₹
                                {getGstPreview()!.gstAmount.toLocaleString(
                                  "en-IN"
                                )}
                              </div>
                              <div>
                                Amount (excluding GST): ₹
                                {getGstPreview()!.baseAmount.toLocaleString(
                                  "en-IN"
                                )}
                              </div>
                            </>
