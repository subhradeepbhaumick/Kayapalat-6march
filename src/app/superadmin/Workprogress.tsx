...
export default function WorkProgress() {
    ...
    const fetchDashboard = async () => {
        try {
            setLoading(true);
            const res = await fetch(`/api/superadmin/workprogress?date=${selectedDate}`);
            const data = await res.json();
            if (data.success) {
                setStats(data.stats);
                setWorkProgress(data.employees);
            }
            const callRes = await fetch("/api/employees/per-day-call-kp");
            const callData = await callRes.json();

            if (callData.success) {
                setCallCounts(callData.data);
            }
        } catch (error) {
...
console.log(
  "call keys",
  callCounts.map((x) => x.emp_id)
);

console.log(
  "employee keys",
  filteredEmployees.map((x) => x.id)
);

console.log(
  "work type keys",
  filteredEmployees.flatMap((e) => e.work_types)
);
    return (
                    ...
                    <div className="flex flex-wrap items-center gap-2 rounded-xl border bg-white p-3 shadow-sm">
                        ...
                        {callCounts.map((emp) => (
                            ...
                            >
                                ...
                                <p
                                    className={`font-bold ${emp.total_count >= 75
                                            ? "text-green-600"
                                            : "text-orange-600"
                                        }`}
                                >
