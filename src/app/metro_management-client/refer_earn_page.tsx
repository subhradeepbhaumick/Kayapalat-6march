...
const ReferAndEarnPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchClients = async () => {
      const res = await fetch(`/api/metro/metro_client/leads?agent_id=${session.user.id}`, {
        credentials: "include",
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch("/api/metro/metro_client/leads", {
      method: "POST",
    ...
    if (res.ok) {
      const updated = await fetch(
        `/api/metro/metro_client/leads?agent_id=${session?.user?.id}`
      );
