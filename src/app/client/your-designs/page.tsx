...
const YourDesignsPage: React.FC = () => {
  ...
  const [selectedRoomForComment, setSelectedRoomForComment] = useState<RoomWithLatestRevision | null>(null);
  const [comments, setComments] = useState<DesignComment[]>([]);
  const [commentText, setCommentText] = useState('');
  const [isSubmittingComment, setIsSubmittingComment] = useState(false);
  const [loadingComments, setLoadingComments] = useState(false);
  const [showPreviousRevisions, setShowPreviousRevisions] = useState<{ [roomId: number]: boolean }>({});
  const [previousRevisions, setPreviousRevisions] = useState<{ [roomId: number]: any[] }>({});
  const [currentDesignIndices, setCurrentDesignIndices] = useState<{ [roomId: number]: number }>({});
  ...
  const [visibleRevisions, setVisibleRevisions] = useState<Set<string>>(new Set());
  ...
  useEffect(() => {
    ...
    const fetchFinalized = async () => {
      try {
        const res = await fetch('/api/client/finalize-designs', { credentials: 'include' });
        if (res.ok) {
    ...
    if (selectedRoomForComment) {
      const fetchComments = async () => {
        ...
        try {
          ...
          const designIds = selectedRoomForComment.latest_revision.designs.map(d => d.id);
          const commentsPromises = designIds.map(id =>
            fetch(`/api/client/designs/${id}/comments`, { credentials: 'include' })
          ...
          allComments.sort((a, b) => new Date(b.date + ' ' + b.time).getTime() - new Date(a.date + ' ' + a.time).getTime());
  ...
  const handleSubmitComment = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/client/designs/${firstDesign.id}/comments`, {
        method: 'POST',
        headers: {
  ...
  --------------------------------------------------------- */
  return (
    <div className="min-h-screen bg-[#D2EBD0] p-4 sm:p-8">
      <style jsx global>{`
        * {
          user-select: none;
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-4">
          ...
          {!finalized && rooms.length > 0 && (
            <button
              onClick={() => {
                const proceed = window.confirm('Attention — finalizing your design cannot be reverted or modified later. Are you sure you want to proceed?');
        ...
        <div className="text-center mb-8">
          ...
          <div className="mt-2 text-sm text-gray-700 flex items-center justify-center gap-2">
            <span>
              You will receive a maximum of <strong>3 design sets</strong> to choose from.
            </span>
            <button
              type="button"
        ...
        {finalized ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {finalizedSelections.map((s: any) => (
              <div key={`${s.room_id}-${s.design_id}`} className="bg-white rounded-xl shadow-md p-4">
                <h3 className="font-semibold text-[#295A47] mb-2">{s.room_name}</h3>
                <div className="aspect-video overflow-hidden rounded">
                  <img src={`/api/images/resolve?path=${s.image_path}`} className="w-full h-full object-cover" alt="final" />
                </div>
              </div>
            ))}

            <div className="fixed bottom-6 right-6">
              <button onClick={() => window.location.href = '/client/your-designs?view=past'} className="px-4 py-2 bg-[#295A47] text-white rounded-lg">See Your Past Designs</button>
            </div>
          </div>
        ) : rooms.length === 0 ? (
            ...
            {rooms.map((room) => (
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-xl font-bold text-[#295A47] truncate">{room.room_name}</h2>
                  <div className="flex gap-2">
                    ...
                    >
                      {room.latest_revision.can_comment ? 'Add Remarks' : 'Comments Locked'}
                    </button>
                    <button
                      onClick={() => {
                        ...
                        if (!previousRevisions[roomId]) {
                          ...
                          fetch(`/api/client/rooms/${roomId}/revisions`, { credentials: 'include' })
                            .then(res => res.json())
                            .then(data => {
                    ...
                    >
                      {showPreviousRevisions[room.room_id] ? 'Hide Previous' : 'View Previous Designs'}
                    </button>
                  </div>
                </div>

                <div className="text-sm text-gray-600 mb-4">
                  Revision {room.latest_revision.revision_number} of 3 • {new Date(room.latest_revision.created_at).toLocaleDateString()}
                </div>
                    ...
                    <>
                      <div className="h-64 relative mb-4">
                        <img
                          ...
                          alt={`${currentDesign.room_name} - ${currentDesign.product_name}`}
                          className="w-full h-full object-cover cursor-pointer"
                          onClick={() => setActiveImage(currentDesign.image_path)}
                          draggable={false}
                          onContextMenu={(e) => e.preventDefault()}
                        />

                        {/* Dense watermark overlay (UNCHANGED) */}
                      ...
                      <div className="flex justify-between items-center mb-2">
                        {designs.length > 1 && (
                          <button
                            onClick={() => setCurrentDesignIndices(prev => ({ ...prev, [room.room_id]: Math.max(0, (prev[room.room_id] || 0) - 1) }))}
                        ...
                        <span className="text-gray-700 font-medium text-center flex-1">{currentDesign.product_name}</span>
                        {designs.length > 1 && (
                          <button
                            onClick={() => setCurrentDesignIndices(prev => ({ ...prev, [room.room_id]: Math.min(designs.length - 1, (prev[room.room_id] || 0) + 1) }))}
                            className="px-2 py-1 bg-gray-200 rounded text-sm hover:bg-gray-300 disabled:opacity-50"
                      ...
                      <div className="flex justify-between items-center text-xs text-gray-500">
                        ...
                        <span>{new Date(currentDesign.timestamp).toLocaleDateString()}</span>
                      </div>
                    </>
                  );
                })()}

                {showPreviousRevisions[room.room_id] && previousRevisions[room.room_id] && (
                  <div className="mt-6 border-t pt-6">
                    <h3 className="text-lg font-semibold mb-4">Previous Revisions</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                      {previousRevisions[room.room_id].flatMap((revision: any) =>
                        ...
                        >
                          <div className="aspect-square relative">
                            <img
                              ...
                              className="w-full h-full object-cover cursor-pointer"
                              onClick={() => setActiveImage(design.image_path)}
                              draggable={false}
                              onContextMenu={(e) => e.preventDefault()}
                            />
                          ...
                          <div className="p-3">
                            <p className="text-sm font-medium">
                              Revision {design.revision_number} - {design.product_name}
                            </p>
                            <p className="text-xs text-gray-500">
                              {new Date(design.revision_date).toLocaleDateString('en-IN')}
                            </p>
                          </div>
                        </div>
                      ))}
          ...
          <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center">
            <div className="bg-white max-w-md w-full rounded-xl p-6 shadow-lg">
              ...
              <p className="text-sm text-gray-700 leading-relaxed">
                ...
                <strong> no further changes are allowed after the third revision</strong>.
                <br /><br />
                If additional changes are required, <strong>a new design fee will apply</strong>.
              </p>
