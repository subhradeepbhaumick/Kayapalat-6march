...
import { Upload, Plus, Edit, Trash2, Filter, ArrowUpDown, MessageCircle } from 'lucide-react';
...
const UploadDesignsTable: React.FC<UploadDesignsTableProps> = () => {
  ...
  const [selectedDesignForReview, setSelectedDesignForReview] = useState<Design | null>(null);
  const [reviewComments, setReviewComments] = useState<any[]>([]);
  const [loadingReview, setLoadingReview] = useState(false);
  const [selectedImageForPreview, setSelectedImageForPreview] = useState<{ path: string; alt: string } | null>(null);
  const [currentEditingDesignId, setCurrentEditingDesignId] = useState<number | null>(null);
  const [currentImage, setCurrentImage] = useState<{ path: string; alt: string } | null>(null);
  const [showEditRoomModal, setShowEditRoomModal] = useState(false);
  const [selectedRoom, setSelectedRoom] = useState<RoomWithRevisions | null>(null);
  ...
  const fetchRooms = async () => {
    try {
      ...
      const response = await fetch('/api/designer/rooms', { credentials: 'include' });
      if (response.ok) {
        ...
        const response = await fetch('/api/designer/designs', { credentials: 'include' });
        if (response.ok) {
          ...
          await Promise.all(designs.map((design: Design) => fetchComments(design.id)));
        }
      }
    } catch (error) {
  ...
  const fetchComments = async (designId: number) => {
    try {
      const response = await fetch(`/api/client/designs/${designId}/comments`, { credentials: 'include' });
      if (response.ok) {
        const data = await response.json();
  ...
  const filteredAndSortedRooms = rooms
    ...
    .sort((a, b) => {
      const dateA = new Date(a.revisions[a.revisions.length - 1]?.created_at || 0).getTime();
      const dateB = new Date(b.revisions[b.revisions.length - 1]?.created_at || 0).getTime();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!editingDesign && !formData.image) {
    ...
    try {
      const url = editingDesign ? `/api/designer/designs/${editingDesign.id}` : '/api/designer/designs';
      ...
      if (response.ok) {
        toast.success(editingDesign ? 'Design updated successfully' : 'Design uploaded successfully');
  ...
  const handleDeleteRoom = async (room: RoomWithRevisions) => {
    if (!confirm(`Are you sure you want to delete the entire room "${room.room_name}" and all its designs? This action cannot be undone.`)) return;
        ...
        <table className="w-full border-collapse border border-gray-300">
          <thead>
            <tr className="bg-[#D7E7D0]">
              <th className="border border-gray-300 px-2 sm:px-4 py-2 text-left">Client Name</th>
              <th className="border border-gray-300 px-2 sm:px-4 py-2 text-left">Room Name</th>
              <th className="border border-gray-300 px-2 sm:px-4 py-2 text-center">Design 1</th>
              <th className="border border-gray-300 px-2 sm:px-4 py-2 text-center hidden sm:table-cell">Design 2</th>
              <th className="border border-gray-300 px-2 sm:px-4 py-2 text-center hidden sm:table-cell">Design 3</th>
          ...
          <tbody>
            {filteredAndSortedRooms.map((room) => {
              ...
              const design1 = room.revisions.find(r => r.revision_number === 1)?.designs || [];
              const design2 = room.revisions.find(r => r.revision_number === 2)?.designs || [];
              const design3 = room.revisions.find(r => r.revision_number === 3)?.designs || [];

              return (
                <tr key={room.room_id} className="hover:bg-gray-50">
                  <td className="border border-gray-300 px-2 sm:px-4 py-2">{room.client_name}</td>
                  <td className="border border-gray-300 px-2 sm:px-4 py-2 font-medium">{room.room_name}</td>
                  ...
                  <td className="border border-gray-300 px-2 py-2">
                    <div className="flex flex-wrap gap-1 justify-center">
                        ...
                        <div key={design.id} className="relative group">
                          ...
                          <button
                            onClick={(e) => {
                              ...
                              fetch(`/api/client/designs/${design.id}/comments`, { credentials: 'include' })
                                .then(res => res.json())
                                .then(data => {
                          ...
                          {comments[design.id] && comments[design.id].length > 0 && (
                      ...
                      {design1.length > 3 && (
                        <span className="text-xs text-gray-500">+{design1.length - 3}</span>
                      )}
                    </div>
                  </td>
                  ...
                  <td className="border border-gray-300 px-2 py-2 hidden sm:table-cell">
                    <div className="flex flex-wrap gap-1 justify-center">
                      ...
                      {design2.length > 3 && (
                        <span className="text-xs text-gray-500">+{design2.length - 3}</span>
                      )}
                    </div>
                  </td>
                  ...
                  <td className="border border-gray-300 px-2 py-2 hidden sm:table-cell">
                    <div className="flex flex-wrap gap-1 justify-center">
                      ...
                      {design3.length > 3 && (
                        <span className="text-xs text-gray-500">+{design3.length - 3}</span>
                      )}
                    </div>
                  </td>
